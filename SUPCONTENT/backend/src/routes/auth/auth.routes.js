...
import {
  assertGoogleCallbackSupported,
  buildOAuthState,
  getGoogleCallbackUrl,
  getGoogleOAuthConfigError,
} from '../../services/auth/oauthFlow.js';
const router = Router();
...
router.get('/google', (req, res, next) => {
  ...
  const callbackError = assertGoogleCallbackSupported(callbackURL);
  ...
  if (state.error) {
    return res.status(400).json({ message: state.error });
  }
