...
const CommentIcon = () => (
  <svg viewBox="0 0 20 20" fill="none" className="w-5 h-5">
    <path
      d="M4 14.5l-1.5 2.2V5A2 2 0 014.5 3h11A2 2 0 0117.5 5v8.5a2 2 0 01-2 2H4z"
      stroke="currentColor"
      strokeWidth="1.5"
      strokeLinejoin="round"
    />
...
function Avatar({ user, size = "sm" }) {
  ...
  return (
    <div
      className={`${dim} rounded-full overflow-hidden bg-gray-100 dark:bg-gray-800 flex items-center justify-center flex-shrink-0 border border-gray-200 dark:border-gray-700`}
    >
      {user?.avatar_url ? (
        <img
          src={user.avatar_url}
        ...
        <span className={`${txt} font-bold text-gray-500 dark:text-gray-400`}>
          {initials}
        </span>
      )}
    </div>
...
function MovieCard({ movie }) {
  ...
  if (movieHref) {
    return (
      <Link
        to={movieHref}
        className="group flex gap-3 bg-gray-50 dark:bg-gray-800/60 rounded-xl p-3 hover:bg-gray-100 dark:hover:bg-gray-800 transition-colors"
      >
        {content}
      </Link>
    );
